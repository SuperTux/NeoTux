...
bool get_line_from_slope_metas(uint16_t meta, Vec2 *lines);

Vec2 get_normal_from_slope_metas(uint16_t meta);
...
get_chunk_collisions(Rectf box, long chunk_size)
{
	...
	return Rectf{ (float) bl, (float) bt, (float) br, (float) bb };
}
...
template<typename T>
std::optional<Vec2_t<T>>
line_line(Vec2_t<T> l1_beg, Vec2_t<T> l1_end, Vec2_t<T> l2_beg, Vec2_t<T> l2_end)
{
	const T dnom = ((l2_end.y - l2_beg.y) * (l1_end.x - l1_beg.x) -
	                (l2_end.x - l2_beg.x) * (l1_end.y - l1_beg.y));
	T dist1      = ((l2_end.x - l2_beg.x) * (l1_beg.y - l2_beg.y) -
               (l2_end.y - l2_beg.y) * (l1_beg.x - l2_beg.x)) /
	          dnom;
	T dist2 = ((l1_end.x - l1_beg.x) * (l1_beg.y - l2_beg.y) -
	           (l1_end.y - l1_beg.y) * (l1_beg.x - l2_beg.x)) /
	          dnom;

	// Lines are between 0 and 1
	if (dist1 == std::clamp<T>(dist1, 0, 1) && dist2 == std::clamp<T>(dist2, 0, 1))
	{
...
// Sometimes there are reports of 3 collisions between a line. To be safe, I set this to 4 (all
// sides), though
...
template<typename T>
size_t
line_rect(col_side_t<T> col_list[], Vec2_t<T> l_beg, Vec2_t<T> l_end, Rect_t<T> rect)
{
	size_t cols = 0;
