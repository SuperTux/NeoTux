...
LevelScreen::draw()
{
	...
	TextureRef lvlname = g_font_manager.load(SUPERTUX_MEDIUM, 30,
	                                         std::format("{}", m_level->get_name()),
	                                         { 255, 220, 220, 225 });
	TextureRef lives   = g_font_manager.load(SUPERTUX_MEDIUM, 30,
	                                         std::format("Lives x {}", g_stats.lives),
	                                         { 255, 220, 220, 255 });

	painter->draw_fill_rect({ 0, 0, (float) camera.width, (float) camera.height },
	                        { 0, 0, 0, 255 });

	painter->draw(lvlname, std::nullopt,
	              Rectf{
	                  (float) (camera.width / 2.f - lvlname->get_size().width / 2.f),
	                  (float) (camera.height / 2.f - lvlname->get_size().height / 2.f) -
	                      lvlname->get_size().height,
	...
	painter->draw(lives, std::nullopt,
	              Rectf{ (float) (camera.width / 2.f - lives->get_size().width / 2.f),
	                     (float) (camera.height / 2.f - lives->get_size().height / 2.f) +
	                         lives->get_size().height,
