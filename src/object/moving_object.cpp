...
{
	Rectf res{ m_rect.left + m_colbox.left, m_rect.top + m_colbox.top,
                           ...
                           std::optional<Rectf> custom_colbox)
{
	...
	if (collide.is_colliding() && do_real_collision_stuff)
	{
		...
		{
			std::cout << collide.top_constraint << " " << collide.left_constraint << " "
			          << collide.right_constraint << " " << collide.bottom_constraint << std::endl;
			if (collide.top_constraint == 32 && collide.right_constraint == 32 &&
			    collide.bottom_constraint == 32 && collide.left_constraint == 32)
			{
				move(0, -32);
			} else
...
void
MovingObject::do_slope_collision(uint16_t sl_t, Vec2 line_beg, Vec2 line_end,
                                 bool do_real_collision_stuff, std::optional<Rectf> custom_colbox)
{
...
{
	float old_width = m_rect.get_width(), old_height = m_rect.get_height();
	m_rect.left   = x;
