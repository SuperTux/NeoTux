...
MovingSprite::parse_sprite()
{
	...
	bool not_new = false;
	std::unique_ptr<SDL_Surface, decltype(&SDL_DestroySurface)> surface{ nullptr,
		                                                                 SDL_DestroySurface };
...
#ifdef NEOTUX_BLIT_TEXTURES
					//surface.reset(Texture::create_surface(FS::path(m_parent_dir + '/' +
					//m_spritesheet)));
#endif
				}
			}
		...
		if (elt.get_value() == "action")
		{
			...
			if (aelt && !m_spritesheets.empty())
			{
				while (aelt.next_inplace())
				{
					...
					_blitter.blit(surface.get(), sheet.to_sdl_rect(),
			...
			sprite_action = new SpriteAction(fps, loops, images, spritesheets, spritesheet_idx,
			                                 scale, hitboxes);
...
#else
		TextureRef tex = g_texture_manager.load(m_parent_dir + "/" +
		                                        m_spritesheets[m_action->spritesheet_idx]);
