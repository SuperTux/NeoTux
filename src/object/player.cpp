...
Player::controls_move(bool right)
{
	...
	} else if (m_physics.get_x_vel() < -0.1 && right || m_physics.get_x_vel() > 0.1 && !right)
	{
		m_physics.set_x_vel(100 * dir);
	}
	...
	{
		m_physics.set_x_vel(std::clamp(m_physics.get_x_vel(), -MAX_RUN_XM - initial_speed,
		                               MAX_RUN_XM + initial_speed));
	}

	if (!(m_slope_normals.size() > 1 && m_slope_normals[1].x + m_slope_normals[0].x == 0.0))
	{
		for (auto &slope_normal : m_slope_normals)
		{
			...
			{
				move(slope_rotated.x * -dir * 0.5 * g_dtime,
				     slope_rotated.y * -dir * 0.5 * g_dtime);
				already_moved = true;
			}
		}
	} else
...
Player::update(Sector &sector, Tilemap &tilemap)
{
	...
	if (!m_state.get(PLAYER_MOVING) &&
	    m_physics.get_x_vel() == std::clamp<double>(m_physics.get_x_vel(), -35, 35))
	{
...
Player::draw()
{
	...
	g_video_system->get_painter()->draw_line(colbox, colbox + m_slope_normal * 60,
	                                         { 0, 255, 0, 255 });
	MovingSprite::draw();
}
