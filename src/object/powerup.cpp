...
Powerup::draw()
{
	...
	TextureRef tex    = g_texture_manager.load(m_parent_dir + "/" +
	                                           m_action->get_image(m_action_timer));
	float crop_height = (m_popout_timer.get_percentage() / 100) * tex->get_size().height;
	Rectf crop_sprite{ 0,
		               -tex->get_size().height + crop_height,
		               { (float) tex->get_size().width, (float) tex->get_size().height } };
