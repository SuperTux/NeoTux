...
SurfaceBlitter::read_pixel(int x, int y)
{
	...
	Uint8 *p;
	p = reinterpret_cast<Uint8 *>(surface->pixels) + y * surface->pitch +
...
void
SurfaceBlitter::blit(SDL_Surface *surface, std::optional<SDL_Rect> src,
                     std::optional<SDL_Rect> dest)
{
	SDL_BlitSurfaceScaled(surface, src ? &*src : NULL, m_sdl_surface.get(), dest ? &*dest : NULL,
