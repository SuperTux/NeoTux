...
CollisionTest::run()
{
	...
	TextureRef help_1 = g_font_manager.load(SUPERTUX_MEDIUM, 32, "Left click to move relative",
	                                        { 255, 255, 255, 155 });
	painter->draw(help_1, std::nullopt,
	              Rectf{ 0,
	                     0,
	                     { (float) help_1->get_size().width, (float) help_1->get_size().height } });
	TextureRef help_2 = g_font_manager.load(SUPERTUX_MEDIUM, 32, "Middle click to move camera",
	                                        { 255, 255, 255, 155 });
	painter->draw(help_2, std::nullopt,
	              Rectf{ 0,
	                     (float) help_1->get_size().height * 2,
	                     { (float) help_2->get_size().width, (float) help_2->get_size().height } });
	TextureRef help_3 = g_font_manager.load(SUPERTUX_MEDIUM, 32,
	                                        "Right click to move Tux to cursor",
	                                        { 255, 255, 255, 155 });
	painter->draw(help_3, std::nullopt,
	              Rectf{ 0,
	                     (float) help_1->get_size().height * 4,
	                     { (float) help_3->get_size().width, (float) help_3->get_size().height } });
	...
	if (rectcol)
	{
		...
		for (size_t i = 0; i < rectcol; ++i)
		{
			...
			{
			case RECT_LEFT:
				painter->draw_line({ colbox.left, colbox.top }, { colbox.left, colbox.bottom },
				                   lcolor);
				break;
			case RECT_RIGHT:
				painter->draw_line({ colbox.right, colbox.top }, { colbox.right, colbox.bottom },
				                   lcolor);
				break;
			case RECT_TOP:
				painter->draw_line({ colbox.left, colbox.top }, { colbox.right, colbox.top },
				                   lcolor);
				break;
			case RECT_BOTTOM:
				painter->draw_line({ colbox.left, colbox.bottom }, { colbox.right, colbox.bottom },
				                   lcolor);
				break;
			}
		}
