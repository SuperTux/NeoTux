...
draw_textures()
{
	...
	for (const auto &pair : g_texture_manager.m_textures)
	{
		TextureRef tex = pair.second.get();
		SDL_FRect dest = { (float) ((i * 40) % winsize.width),
			               (float) ((i * 40) / winsize.width) * 40, 40, 40 };
		//SDL_RenderTexture(static_cast<SDLVideoSystem*>(g_video_system.get())->m_sdl_renderer.get(),
		//texture, NULL, &dest);
...
EverythingTest::run()
{
		...
		for (int y = 0; y < tilemap->get_size().height; ++y)
		{
			...
			if (tile.get_id() != 0)
			{
				try
				{
					...
					painter->draw(tex, tilemeta.get_src_rect(tex),
	...
	painter->draw(text, std::nullopt,
	              SDL_FRect{ 50, 50, (float) text->get_size().width,
	                         (float) text->get_size().height });
	...
	//#ifndef NDEBUG
	TextureRef fps = g_font_cache.load(std::format("FPS: {}", (int) get_fps()),
	                                   { 255, 255, 255, 155 });
	painter->draw(fps, std::nullopt,
	              Rectf{ 0, 0, { (float) fps->get_size().width, (float) fps->get_size().height } });
	TextureRef draws = g_font_cache.load(std::format("Draws: {}", painter->get_draw_count()),
	                                     { 255, 255, 255, 255 });
	painter->draw(draws, std::nullopt,
	              SDL_FRect{ 0, (float) fps->get_size().height, (float) draws->get_size().width,
	                         (float) draws->get_size().height });
	TextureRef total_draws = g_font_cache.load(std::format("Total Draws: {}",
	                                                       painter->get_total_draw_count()),
	                                           { 255, 255, 255, 255 });
	painter->draw(total_draws, std::nullopt,
	              SDL_FRect{ 0, (float) draws->get_size().height * 2,
	                         (float) total_draws->get_size().width,
	                         (float) total_draws->get_size().height });
	//#endif
