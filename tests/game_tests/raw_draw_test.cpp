...
RawDrawTest::run()
{
	...
	// state
	double total = 3.0;
	ViewContext cam{ (unsigned) g_video_system->get_window_size().width,
		             (unsigned) g_video_system->get_window_size().height };
	bool jumped      = false;
	...
	TextureRef fpstex = g_font_manager.load(SUPERTUX_MEDIUM, 18,
	                                        std::format("({}) FPS: {:.0f}",
	                                                    use_painter ? "Painter" : "SDL", get_fps()),
	                                        { 255, 255, 255, 155 });
	...
	for (int i = 0; i < ceil(cam.width / (32 * 15)) + 1; ++i)
		for (int j = 0; j < ceil(cam.height / (32 * 15)) + 1; ++j)
		{
			painter->draw(tileset_tex.get(), std::nullopt,
			              SDL_FRect{ (float) i * (32 * 15), (float) j * (32 * 15),
			                         (float) tileset_tex->get_size().width,
			                         (float) tileset_tex->get_size().height });
		}
	...
	for (int i = 0; i < (int) total; ++i)
	{
		...
		{
			SDL_FRect dest = { (float) (((i * 12) * cam.zoom) + cam.x),
				               (float) (((j * 12) + cam.y) * cam.zoom), (float) (tex->w * cam.zoom),
